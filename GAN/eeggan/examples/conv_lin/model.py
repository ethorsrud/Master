...
def create_disc_blocks(n_chans,base,conditional):
	...
	def create_in_sequence(n_chans,out_filters):
		return nn.Sequential(weight_scale(nn.Conv2d(2,out_filters,(1,n_chans)),
														gain=calculate_gain('leaky_relu')),
...
def create_gen_blocks(n_chans,z_vars,conditional):
	...
	tmp_block = ProgressiveGeneratorBlock(
								nn.Sequential(Reshape([[0],57,-1]),
								weight_scale(nn.Conv1d(57,n_featuremaps,101,padding=50,stride=4),
														gain=calculate_gain('leaky_relu')),
												nn.LeakyReLU(0.2),
								weight_scale(nn.Conv1d(n_featuremaps,n_featuremaps,51,padding=25,stride=4),
														gain=calculate_gain('leaky_relu')),
												nn.LeakyReLU(0.2),
								weight_scale(nn.Conv1d(n_featuremaps,n_featuremaps,9,padding=4,stride=4),
														gain=calculate_gain('leaky_relu')),
												nn.LeakyReLU(0.2),
