...
def create_disc_blocks(n_chans,base,conditional):
	...
	blocks.append(tmp_block)
	tmp_block = ProgressiveDiscriminatorBlock(
							  create_conv_sequence(n_featuremaps+conditional,n_featuremaps+conditional),
	...
	tmp_block = ProgressiveDiscriminatorBlock(
							  nn.Sequential(StdMap1d(),
											create_conv_sequence(n_featuremaps+1+conditional,n_featuremaps+conditional),
											Reshape([[0],-1]),
											weight_scale(nn.Linear((n_featuremaps+conditional)*base,1),
