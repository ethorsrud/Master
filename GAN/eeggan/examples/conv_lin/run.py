    ...
    for i_epoch in range(i_epoch_tmp,block_epochs[i_block]):
        ...
        for it in range(iters):
            for i_critic in range(n_critic):
                ...
                batch_real_fft = torch.sqrt(batch_real_fft[:,:,1:,:,0]**2+batch_real_fft[:,:,1:,:,1]**2+1e-16)#batch_real_fft[:,:,:,:,0]**2
                batch_fake_fft = torch.transpose(torch.rfft(torch.transpose(batch_fake,2,3),1,normalized=False),2,3)
                batch_fake_fft = torch.sqrt(batch_fake_fft[:,:,1:,:,0]**2+batch_fake_fft[:,:,1:,:,1]**2+1e-16)#batch_fake_fft[:,:,:,:,0]**2
